...
/* subtle grid texture — z-index:-1 keeps it behind all interactive elements */
.stApp::before {
    content: '';
    position: fixed;
    inset: 0;
...
/* hide only the 'made with streamlit' footer — keep header & menu intact */
...
with tab_upload:
    ...
    if st.button("⬆  UPLOAD & CHUNK DOCUMENTS", disabled=not uploaded):
            ...
            for idx, uf in enumerate(uploaded):
                ...
                try:
                    ...
                    for row in existing.data:
                        pass  # selective delete not done to keep it simple
    ...
    if st.session_state.running:
        ...
        try:
            ...
            for doc_idx, doc_name in enumerate(documents):
                ...
                chunk_meta  = [c.get("metadata",{}) for c in chunks]
                ...
                # evaluate — pass test_cases directly, no EvaluationDataset needed
                eval_result = de_evaluate(test_cases=test_cases, metrics=metrics_list)

                # collect scores from each test case's metrics dict
                doc_scores = {"doc": doc_name, "metrics": {}, "test_cases": len(test_cases)}
                for tc in test_cases:
                    # deepeval stores results in tc.metrics_metadata after evaluate()
                    if hasattr(tc, "metrics_metadata") and tc.metrics_metadata:
                        ...
                        # fallback: read score off each metric object (last-test-case value)
...
with tab_results:
    if not st.session_state.eval_results and not st.session_state.goldens:
        ...
        for doc_result in st.session_state.eval_results:
            ...
            st.markdown(f'<div class="evo-card"><div class="evo-card-title">📄 {doc_name}</div>', unsafe_allow_html=True)
